...
<template>
  ...
  <template v-else>
    ...
    <section class="mb-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_50px_rgb(15_23_42/0.055)] dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none">
      <div class="border-b border-slate-100 bg-gradient-to-r from-teal-50 via-white to-sky-50 px-5 py-4 dark:border-zinc-800 dark:from-teal-900/20 dark:via-zinc-900 dark:to-sky-900/20">
        <h2 class="text-lg font-extrabold">Today Workspace</h2>
        <p class="text-sm font-medium text-slate-500 dark:text-zinc-400">Key signals for the current workday.</p>
      </div>
      <div class="grid grid-cols-2 gap-3 p-4 md:grid-cols-4">
        <div class="rounded-2xl border border-teal-100 bg-teal-50/70 p-4 dark:border-teal-800/30 dark:bg-teal-900/10"><p class="label">Progress</p><p class="mt-3 text-2xl font-extrabold text-teal-800 sm:text-3xl dark:text-teal-300">{{ data.summary.today_progress }}</p><p class="mt-1 text-xs font-semibold text-teal-700 dark:text-teal-500">daily entries</p></div>
        <div class="rounded-2xl border border-sky-100 bg-sky-50/70 p-4 dark:border-sky-800/30 dark:bg-sky-900/10"><p class="label">Open tasks</p><p class="mt-3 text-2xl font-extrabold text-sky-800 sm:text-3xl dark:text-sky-300">{{ data.summary.open_tasks }}</p><p class="mt-1 text-xs font-semibold text-sky-700 dark:text-sky-500">needs attention</p></div>
        <div class="rounded-2xl border border-rose-100 bg-rose-50/70 p-4 dark:border-rose-800/30 dark:bg-rose-900/10"><p class="label">Blockers</p><p class="mt-3 text-2xl font-extrabold text-rose-800 sm:text-3xl dark:text-rose-300">{{ data.summary.blockers }}</p><p class="mt-1 text-xs font-semibold text-rose-700 dark:text-rose-500">active friction</p></div>
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/40"><p class="label">Learning</p><p class="mt-3 text-2xl font-extrabold text-slate-900 sm:text-3xl dark:text-zinc-100">{{ minutes(data.summary.learning_minutes_this_week) }}</p><p class="mt-1 text-xs font-semibold text-slate-500 dark:text-zinc-500">this week</p></div>
      </div>
    </section>
    <div class="grid gap-6 xl:grid-cols-3">
      <section class="card p-5 xl:col-span-1"><div class="mb-4 flex items-center justify-between"><h2 class="font-extrabold">Weekly activity</h2><span class="rounded-full bg-teal-50 px-2.5 py-1 text-xs font-bold text-teal-700 dark:bg-teal-900/30 dark:text-teal-400">7 days</span></div><div class="flex h-44 items-end gap-1.5 rounded-2xl bg-slate-50 p-3 dark:bg-zinc-800/40"><div v-for="item in data.weekly_activity" :key="item.date" class="flex flex-1 flex-col items-center gap-2"><div class="w-full rounded-t-lg bg-gradient-to-t from-teal-700 to-sky-400 shadow-sm" :style="{ height: `${Math.max(6, (item.work + item.learning + item.progress) * 24)}px` }" /><span class="text-[10px] font-bold text-slate-500 dark:text-zinc-500">{{ item.date.slice(8) }}</span></div></div></section>
      <section class="card p-5"><h2 class="mb-4 font-extrabold">Latest work</h2><div class="space-y-3"><RouterLink v-for="log in data.latest_work_logs" :key="log.id" :to="`/work-logs`" class="block rounded-2xl border border-slate-200 p-3.5 transition hover:border-teal-200 hover:bg-teal-50/40 hover:shadow-sm dark:border-zinc-700 dark:hover:border-teal-700 dark:hover:bg-teal-900/20"><b class="text-slate-900 dark:text-zinc-100">{{ log.title }}</b><p class="mt-1 text-sm font-medium text-slate-500 dark:text-zinc-400">{{ log.project_name }} · {{ minutes(log.actual_duration) }}</p></RouterLink></div></section>
      <section class="card p-5"><h2 class="mb-4 font-extrabold">Projects</h2><div class="space-y-2"><RouterLink v-for="project in data.projects" :key="project.id" :to="`/projects/${project.id}`" class="block rounded-2xl border border-slate-200 p-3.5 transition hover:border-teal-200 hover:bg-teal-50/40 hover:shadow-sm dark:border-zinc-700 dark:hover:border-teal-700 dark:hover:bg-teal-900/20"><b class="text-slate-900 dark:text-zinc-100">{{ project.name }}</b><p class="mt-1 text-sm font-medium text-slate-500 dark:text-zinc-400">{{ project.open_tasks_count }} open tasks</p></RouterLink></div></section>
    </div>
    ...
    <div class="mt-5 grid gap-6 xl:grid-cols-3">
      <section class="card p-5 xl:col-span-2"><div class="mb-4 flex items-center justify-between"><h2 class="font-extrabold">Monthly rhythm</h2><span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-500 dark:bg-zinc-800 dark:text-zinc-400">heatmap</span></div><div class="grid grid-cols-7 gap-1 rounded-2xl bg-slate-50 p-3 dark:bg-zinc-800/40"><div v-for="item in data.monthly_activity" :key="item.date" class="h-8 rounded-lg transition hover:ring-2 hover:ring-teal-200" :class="(item.work + item.learning + item.progress) > 2 ? 'bg-teal-700 dark:bg-teal-500' : (item.work + item.learning + item.progress) > 0 ? 'bg-teal-300 dark:bg-teal-700' : 'bg-white border border-slate-200 dark:bg-zinc-800 dark:border-zinc-700'" :title="item.date" /></div></section>
      <section class="card p-5"><h2 class="mb-4 font-extrabold">Weekly review</h2><div v-if="report" class="space-y-3 text-sm"><div class="rounded-2xl border border-slate-200 bg-slate-50 p-3.5 dark:border-zinc-700 dark:bg-zinc-800/40"><b>Work delta</b><p class="mt-1 text-slate-500 dark:text-zinc-400">{{ report.trends.completed_work_delta }} completed logs vs previous period</p></div><div class="rounded-2xl border border-slate-200 bg-slate-50 p-3.5 dark:border-zinc-700 dark:bg-zinc-800/40"><b>Learning delta</b><p class="mt-1 text-slate-500 dark:text-zinc-400">{{ minutes(report.trends.learning_minutes_delta) }} vs previous period</p></div><RouterLink class="btn btn-primary w-full" to="/reports/weekly">Open report</RouterLink></div></section>
    </div>
    <!-- Habits today + Active Goals + Bills -->
    <div class="mt-5 grid gap-6 xl:grid-cols-3">
      <section class="card p-5">
        <div class="mb-4 flex items-center justify-between">
      ...
      <section class="card p-5">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="font-extrabold">Tagihan Bulan Ini</h2>
          <RouterLink class="text-sm font-extrabold text-teal-700 hover:underline dark:text-teal-400" to="/bills">Lihat semua</RouterLink>
        </div>
        <template v-if="data.bills_summary && data.bills_summary.total_count > 0">
          <div class="mb-4 flex items-end gap-2">
            <span class="text-3xl font-extrabold" :class="data.bills_summary.unpaid_count > 0 ? 'text-amber-700 dark:text-amber-400' : 'text-teal-800 dark:text-teal-400'">{{ data.bills_summary.unpaid_count }}</span>
            <span class="mb-0.5 text-sm font-semibold text-slate-400 dark:text-zinc-500">/ {{ data.bills_summary.total_count }} belum lunas</span>
          </div>
          <div class="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-zinc-800">
            <div class="h-full rounded-full bg-teal-600 transition-all" :style="{ width: `${data.bills_summary.total_count ? Math.round((data.bills_summary.total_count - data.bills_summary.unpaid_count) / data.bills_summary.total_count * 100) : 0}%` }" />
          </div>
          <div class="mt-3 space-y-1.5">
            <div v-if="data.bills_summary.overdue_count > 0" class="flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 dark:bg-red-900/15">
              <svg class="h-3.5 w-3.5 text-red-500" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24"><path d="M12 9v4M12 17h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/></svg>
        ...
        <template v-else-if="data.bills_summary">
          <p class="text-sm text-slate-400 dark:text-zinc-500">Belum ada tagihan. <RouterLink class="font-semibold text-teal-700 hover:underline dark:text-teal-400" to="/bills">Tambah tagihan →</RouterLink></p>
        </template>
      </section>
    </div>

    <section class="card mt-5 p-5">
      <div class="mb-4 flex items-center justify-between"><h2 class="font-extrabold">Milestone progress</h2><RouterLink class="text-sm font-extrabold text-teal-700 hover:underline dark:text-teal-400" to="/milestones">View all</RouterLink></div>
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <RouterLink v-for="milestone in data.milestones" :key="milestone.id" :to="`/milestones/${milestone.id}`" class="rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-teal-200 hover:bg-teal-50/40 dark:border-zinc-700 dark:bg-zinc-900/40 dark:hover:border-teal-700 dark:hover:bg-teal-900/10">
    ...
    <section class="card mt-5 p-5">
      <div class="mb-4 flex items-center justify-between"><h2 class="font-extrabold">Recent activity</h2><span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-500 dark:bg-zinc-800 dark:text-zinc-400">audit trail</span></div>
      <div class="grid gap-2">
        <div v-for="item in data.recent_activity" :key="item.id" class="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-zinc-700 dark:bg-zinc-800/40">
          <div>
            <p class="font-extrabold text-slate-900 dark:text-zinc-100">{{ item.label }}</p>
            <p class="text-sm font-medium text-slate-500 dark:text-zinc-400">{{ item.record_type }} #{{ item.record_id }}</p>
          </div>
          <span class="text-xs font-bold text-slate-400 dark:text-zinc-500">{{ formatDate(item.created_at) }}</span>
        </div>
        <div v-if="data.recent_activity.length === 0" class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm font-semibold text-slate-500 dark:border-zinc-700 dark:bg-zinc-800/40 dark:text-zinc-400">No activity yet.</div>
      </div>
    </section>
  </template>
