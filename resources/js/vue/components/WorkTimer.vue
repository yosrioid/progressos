...
const MODES: Record<TimerMode, { work: number; break: number; label: string }> = {
  free: { work: 0, break: 0, label: 'Bebas' },
...
<template>
  ...
  <div v-else-if="showInput && !elapsed" class="flex items-center gap-1.5">
    ...
    />
    <button :disabled="!label.trim()" class="rounded-lg bg-teal-700 px-2.5 py-1 text-xs font-bold text-white hover:bg-teal-600 disabled:opacity-40" @click="start">Mulai</button>
  ...
  <div v-else class="flex items-center gap-1.5">
    <span v-if="mode !== 'free'" :class="['text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded', isBreak ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400']">
      {{ isBreak ? 'Istirahat' : `#${sessionCount + 1}` }}
    </span>
    <div class="relative">
    ...
    <button v-if="isBreak" class="rounded-lg px-2 py-1 text-[10px] font-bold text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-zinc-800" @click="skipBreak">Lewati</button>
    ...
    <button v-else class="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-teal-700 dark:hover:bg-zinc-800" title="Lanjutkan" @click="start">
    ...
    <button class="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-red-500 dark:hover:bg-zinc-800" title="Buang" @click="discard">
