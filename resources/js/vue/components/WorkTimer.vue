...
function tick() {
  ...
  // Auto-transition when target reached in pomodoro/deep mode
...
function handlePhaseEnd() {
  ...
  running.value = false;
  if (!isBreak.value) {
    // Work phase done — auto-log and start break
    ...
    // Break done — reset for next work session
    isBreak.value = false;
    elapsed.value = 0;
...
function start() {
  if (!label.value.trim()) { showInput.value = true; return; }
...
<template>
  <!-- Idle -->
  <div v-if="!elapsed && !showInput" class="flex items-center gap-1">
    <div class="relative">
      <button
      ...
      <button @click="pomodoroExpanded = !pomodoroExpanded" class="rounded px-1 text-[10px] text-slate-400 hover:text-indigo-500" :title="'Mode: ' + mode">{{ MODES[mode].label }}</button>
      <div v-if="pomodoroExpanded" class="absolute top-full left-0 z-30 mt-1 w-36 rounded-lg border border-slate-200 bg-white shadow-lg dark:border-zinc-700 dark:bg-zinc-900 py-1">
        <button v-for="(m, key) in MODES" :key="key" @click="mode = key as TimerMode; pomodoroExpanded = false"
          :class="['w-full text-left px-3 py-1.5 text-xs font-semibold hover:bg-slate-50 dark:hover:bg-zinc-800', mode === key ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-300']">
          {{ m.label === 'Free' ? 'Free (no limit)' : m.label + ' Pomodoro' }}
        </button>
      </div>
    </div>
  </div>
  ...
  <div v-else-if="showInput && !elapsed" class="flex items-center gap-1.5">
    <input
      v-model="label"
      class="h-7 w-36 rounded-lg border border-slate-200 bg-white px-2 text-xs font-semibold focus:border-teal-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-900"
      placeholder="What are you working on?"
      autofocus
      @keydown.enter="start"
      @keydown.esc="showInput = false"
    />
    <button class="rounded-lg bg-teal-700 px-2.5 py-1 text-xs font-bold text-white hover:bg-teal-600" @click="start">Start</button>
    <button class="rounded-lg px-1.5 py-1 text-xs text-slate-400 hover:text-slate-700" @click="showInput = false">✕</button>
  </div>
  ...
  <div v-else class="flex items-center gap-1.5">
    ...
    <span v-if="!isBreak" class="max-w-[6rem] truncate text-xs font-semibold text-slate-500 dark:text-zinc-400">{{ label }}</span>
    <button v-if="isBreak" @click="skipBreak" class="rounded-lg px-2 py-1 text-[10px] font-bold text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-zinc-800">Skip</button>
