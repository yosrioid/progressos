...
<template>
  ...
  <div v-else class="min-h-screen bg-slate-50 text-slate-950 lg:flex">
    ...
    <div v-if="quick" class="fixed inset-0 z-40 grid place-items-center bg-slate-950/40 p-4">
      <form class="card w-full max-w-xl p-5" @submit.prevent="submitQuick">
        <div class="mb-4 flex items-center justify-between"><h2 class="text-lg font-semibold">Quick Add</h2><button type="button" class="btn btn-muted" @click="quick = false">Close</button></div>
